...
venv/ services/*/venv/ .venv/ services/*/.venv/ __pycache__/ *.pyc
...
## Pitfalls
- **`.venv/` (dot-prefixed) is NOT caught by `venv/` or `services/*venv/`.** Those patterns only
  match a bare `venv` dir, not `.venv`. A service with a dotted venv (e.g. `services/wiki/.venv`,
  1,100+ files / ~15MB) will silently get committed and bloat history. Add explicit `.venv/` and
  `services/*/.venv/` to `.gitignore`. After committing services, ALWAYS sanity-check committed size:
  `git ls-tree -r -l HEAD services/ | awk '{s+=$4} END{print int(s/1024)"KB"}'` — if it's in the
  tens of MB, a venv/blob leaked in; `git rm -r --cached services/<x>/.venv/` (keeps disk files)
  + add the ignore rule + new commit.
